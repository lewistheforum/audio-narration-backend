...
async function bootstrap() {
    ...
    console.log(`🌱 Smart Seeding: ${COUNT} records for entity '${TARGET_ENTITY}'...`);
    ...
    try {
        ...
        if (!metadata) {
            throw new Error(`Instance '${TARGET_ENTITY}' not found in DataSource metadata.`);
        }
...
async function recursiveSeed(dataSource: DataSource, metadata: EntityMetadata, depth = 0): Promise<any> {
    if (depth > 5) {
        throw new Error('Maximum recursion depth exceeded (Circular dependency detected?)');
    }
    ...
    for (const column of metadata.columns) {
        ...
        if (column.isCreateDate || column.isUpdateDate || column.isDeleteDate) continue;
...
function generateFakeData(column: any): any {
    ...
    if (name.includes('amount') || name.includes('price') || name.includes('total')) return faker.number.int({ min: 10000, max: 10000000 });
