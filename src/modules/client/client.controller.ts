...
import { UserRole } from './entities/accounts.entity';
import {
    CreateClientDto,
...
export class ClientController {
    ...
    async findAll(@Query('includeDeleted') includeDeleted?: string): Promise<{ data: ClientResponseDto[]; message: string }> {
    ...
    async findOne(@Param('id', ParseUUIDPipe) id: string): Promise<{ data: ClientResponseDto; message: string }> {
    ...
    async update(
        ...
        const { user, emailChanged } = await this.clientService.update(id, updateClientDto);
    ...
    @ApiResponse({ status: 404, description: 'User not found' })
    async restore(@Param('id', ParseUUIDPipe) id: string): Promise<{ data: ClientResponseDto; message: string }> {
        const client = await this.clientService.restore(id);
        return { data: client, message: MESSAGES.successMessage.userRestoredSuccess };
    }
    ...
    @ApiResponse({ status: 404, description: 'User not found' })
    async unbanUser(@Param('id', ParseUUIDPipe) id: string): Promise<{ data: ClientResponseDto; message: string }> {
        const client = await this.clientService.unbanUser(id);
        return { data: client, message: MESSAGES.successMessage.userUnbannedSuccess };
    }
}
