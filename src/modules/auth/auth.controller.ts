...
export class AuthController {
  ...
  async uploadLegalDocumentsForManager(
  ...
  ): Promise<{ data: LegalDocumentResponseDto; message: string }> {
    const adminId = req.user._id;
    ...
    return {
      data: new LegalDocumentResponseDto(legalDocs),
  ...
  async getLegalDocumentsForManager(
    @Req() req: any,
    @Param('managerAccountId', ParseUUIDPipe) managerAccountId: string,
  ): Promise<{ data: LegalDocumentResponseDto | null; message: string }> {
    const adminId = req.user._id;
    ...
    return {
      data: legalDocs ? new LegalDocumentResponseDto(legalDocs) : null,
      message: 'Legal documents retrieved successfully',
    };
  }
   ...
   * @swagger
   * @security JWT-auth
   * @response 200 - Legal documents updated successfully
   * @response 400 - Cannot update documents with current status
   * @response 401 - Unauthorized
   * @response 403 - Forbidden - Requires CLINIC_ADMIN role or no ownership
   * @response 404 - Clinic manager not found
   */
