...
export class ContractsController {
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Get all contract packages for the logged-in clinic',
  })
  @ApiQuery({
    name: 'employeeName',
    required: false,
  ...
  async getPackages(
    ...
    return this.contractsService.getPackagesByManager(
      clinicManagerId,
      employeeName,
      page,
      limit,
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Get all contract packages for the logged-in employee',
  })
  @ApiQuery({
    name: 'clinicName',
    required: false,
  ...
  async getPackagesByEmployee(
    ...
    return this.contractsService.getPackagesByEmployee(
      employeeId,
      clinicName,
      page,
      limit,
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({
    summary: 'Get contract package by ID for the logged-in employee',
  })
  async getMyContractById(@Req() req, @Param('id') id: string) {
    const employeeId = req.user._id;
  ...
  @ApiResponse({
    status: 201,
    description: 'Contract package created successfully',
  })
  ...
  @ApiResponse({
    status: 201,
    description: 'Contract information created successfully',
  })
  async createContractInfo(
    @Param('id') id: string,
    @Body() dto: CreateContractInfoDto,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Contract package cancelled successfully',
  })
  async cancelPackage(@Param('id') id: string, @Req() req) {
